...
public class ThreadPoolManager implements BeanFactoryAware {
    ...
    final RejectedExecutionHandler handler = new RejectedExecutionHandler() {
        ...
        public void rejectedExecution(Runnable r, ThreadPoolExecutor executor) {
            ...
            msgQueue.offer(((OrderThread) r).getMsg());
        }
    };
    ...
    final ScheduledFuture taskHandler = scheduler.scheduleAtFixedRate(new Runnable() {
        @Override
        public void run() {
            System.out.println("监控是否有订单请求");
            if (!msgQueue.isEmpty()) {
                if (threadPool.getQueue().size() < WORK_QUEUE_SIZE) {
                    System.out.print("调度：");
                    String orderId = (String) msgQueue.poll();
                    OrderThread accessDBThread = (OrderThread) factory.getBean("orderThread");
    ...
    public void processOrders(String orderId) {
        if (cacheMap.get(orderId) == null) {
            ...
            OrderThread accessOrderThread = (OrderThread) factory.getBean("orderThread");
            accessOrderThread.setMsg(orderId);
            threadPool.execute(accessOrderThread);
        }
    }
